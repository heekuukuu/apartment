...
<div class="map_wrap">
    ...
    <div id="menu_wrap" class="bg_white">
        <div class="option">
            <div>
                <form onsubmit="searchPlaces(); return false;">
                    키워드 : <input type="text" value="아파트" id="keyword" size="15">
     ...
     <div style="margin-top: 20px; text-align: center;">
         <button type="button" class="btn btn-primary" onclick="redirectToMain()">Main Page</button>
     </div>
...
function getListItem(index, places) {
    ...
    // 리스트 항목 클릭 시 위치를 저장하는 기능 추가
    el.onclick = function() {
        if (confirm('해당 위치를 저장하시겠습니까?')) {
            saveLocation(places.place_name, places.address_name);  // 위치 저장 함수 호출
        }
    };
...
function saveLocation(name, address) {
    // AJAX 요청을 통해 서버에 아파트 이름과 주소 정보를 저장
    if (!address) {
    ...
    $.ajax({
        type: 'POST',  // 반드시 'POST' 메서드를 사용해야 합니다
        url: '/saveLocation',
        data: JSON.stringify({ apartmentName: name, address: address }),
        contentType: 'application/json; charset=utf-8',
        success: function(response) {
            alert('위치가 저장되었습니다.');
        },
        error: function(xhr, status, error) {
            console.error('Error occurred:', status, error);  // 에러 메시지 출력
