...
<div class="container"> <!-- 메인 컨테이너 시작 -->
    <form action="/api/save" method="post">   <!-- 게시물 저장을 위한 폼 -->
        <!-- 제목 입력 필드 -->
...
$(document).ready(function() {
    $('.summernote').summernote({ // Summernote 초기화
        ...
        placeholder: '최대 10000자까지 쓸 수 있습니다.'  // placeholder 설정
    });
      // 텍스트 길이 제어
            $('#content').on('input', function() {
                ...
                if (content.length > maxLength) {
                    $(this).val(content.substring(0, maxLength)); // 최대 길이를 초과하면 잘라내기
                }
                $('#charCount').text('현재 ' + $(this).val().length + '자 / 최대 ' + maxLength + '자');
      });
       });
</script>
