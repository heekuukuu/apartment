...
<div class="container"> <!-- 메인 컨테이너 시작 -->
    ...
    <c:if test="${boards.user.id == principal.user.id}"> <!-- 현재 로그인한 사용자와 게시물 작성자가 동일할 때 수정, 삭제 버튼을 보여줌 -->
        <a href="/board/updateForm/${boards.id}" class="btn btn-warning">Modify</a> <!-- 수정 버튼 -->
    ...
    <!-- 게시물 세부 정보 출력 -->
    <div>
        Post ID : <span id="id">${boards.id}</span> <br/>
        Post Writer : <span>${boards.user.username}</span> <!-- 작성자 정보 -->
    </div>
...
<script src="/js/board.js"></script> <!-- JavaScript 파일 포함 -->
